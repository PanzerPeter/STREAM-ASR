...
def _run_stage(
    ...
    return handler.run(
        EvaluateCorpus_Command(manifest_path=manifest, mode=mode, ablation_stage=stage, limit=limit)
    )
...
def _tune_alpha(
    ...
    for alpha in grid:
        resp = _run_stage(model, tok, stage, alpha, dev_manifest, "offline", limit)
        marker = ""
    ...
    with torch.no_grad():
        ...
        for stage in cfg.eval.ablation_stages:
            ...
            for mode in ("offline", "streaming"):
                resp = _run_stage(model, tok, stage, lm_weight, args.manifest, mode, args.limit)
                report.append(asdict(resp))
                print(
                    f"{stage:<13} {mode:<9} WER={resp.wer:.4f} CER={resp.cer:.4f} "
