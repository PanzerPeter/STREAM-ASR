...
def test_fresh_run_when_no_checkpoint(tmp_path):
    ...
    assert meta == {"step": 0, "best_wer": float("inf"), "resume_count": 0, "extra": {}}
...
def test_resumes_and_bumps_resume_count(tmp_path):
    ...
    save_checkpoint(
        path, m, [o], step=500, best_wer=0.2, resume_count=0, kind="stage_a", extra={"val_ppl": 7.5}
    )
    m2, o2 = _model_opt()
    ...
    # `extra` rides along so a trainer that selects on something other than WER (the LM: val
    # perplexity) can restore its own best-so-far instead of restarting it at +inf.
    assert meta["extra"] == {"val_ppl": 7.5}
