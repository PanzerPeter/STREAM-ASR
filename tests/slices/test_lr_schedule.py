...
from src.slices.TrainAcousticModel._train_utils import _lr_at

WARMUP, TOTAL = 10_000, 175_000


def _wsd(step: int, **kw) -> float:
    return _lr_at(step, 1.0, WARMUP, TOTAL, schedule="wsd", min_ratio=0.01, **kw)
...
def test_cosine_floor_replaces_the_decay_to_exactly_zero():
    plain = _lr_at(TOTAL, 1.0, WARMUP, TOTAL)
    floored = _lr_at(TOTAL, 1.0, WARMUP, TOTAL, min_ratio=0.01)
    assert plain == 0.0
    ...
    assert math.isclose(_lr_at(80_000, 1.0, WARMUP, TOTAL), mid)
